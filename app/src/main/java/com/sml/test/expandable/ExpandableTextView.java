...
public class ExpandableTextView extends AppCompatTextView {
    private static final int DEF_MAX_LINE = 6;
...
//    public static final String self_regex = "\\[([\\w\\p{InCJKUnifiedIdeographs}-]*)]\\([\\w\\p{InCJKUnifiedIdeographs}-]*\\)";
            ...
            if (currentLines < mLineCount) {
                ...
                ssb.setSpan(new ClickableSpan() {
                    ...
                    public void onClick(View widget) {
                        executeExpanded(StatusType.STATUS_CONTRACT);
                    }
                ...
                if (mNeedContract) {
                    ...
                    ssb.setSpan(new ClickableSpan() {
                        @Override
                        public void onClick(View widget) {
                            executeExpanded(StatusType.STATUS_EXPAND);
                        }
    ...
    public void executeExpanded(StatusType type) {
        if (mModel != null) {
            mModel.setStatus(type);
...
//        Pattern pattern = Pattern.compile(regexp, Pattern.CASE_INSENSITIVE);
        ...
        if (mNeedLink) {
            pattern = PatternsCompat.WEB_URL;
